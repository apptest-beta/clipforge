...
const BADGE_ICON_COLORS = ['text-yellow-300', 'text-orange-300', 'text-amber-300'];

// Reusable Hero Component. The animated shader background is rendered once,
// globally, by <ShaderBackground /> in the root layout - this component is
// just the transparent content overlay.
      ...
      <div className="absolute inset-0 z-10 flex flex-col items-center justify-center text-white">
        ...
        {trustBadge && (
          <div className="mb-8 animate-fade-in-down">
            <div className="hover-glow flex items-center gap-2 px-6 py-3 bg-orange-500/10 backdrop-blur-md border border-orange-300/30 rounded-full text-sm">
              {trustBadge.icons && (
                <div className="flex">
                  {trustBadge.icons.map((icon, index) => (
                    <span key={index} className={BADGE_ICON_COLORS[index % BADGE_ICON_COLORS.length]}>
                      {icon}
                    </span>
                  ))}
                </div>
